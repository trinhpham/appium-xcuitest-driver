...
let tempfileCleanPromise;

async function waitForClearedSystemFiles () {
  if (tempfileCleanPromise) {
    log.debug('Waiting for system files to be cleaned');
    await tempfileCleanPromise;
    tempfileCleanPromise = null;
  }
}
...
async function clearSystemFiles (wda) {
  ...
  tempfileCleanPromise = B.resolve(fs.walkDir(tmpDir, true, function callback (itemPath, isDir) {
    if (isDir) {
      return false;
    }
    ...
    fs.unlink(itemPath)
      ...
      .then(() => cleanedFileCount++)
      // eslint-disable-next-line promise/prefer-await-to-callbacks
      .catch((err) => log.warn(`Error deleting '${itemPath}': ${err.message}`));
  })).finally(function () {
    log.info(`Removed ${cleanedFileCount} temporary XCTest log ` +
      `${pluralize('file', cleanedFileCount)}`);
  }).catch(() => {});
...
export {
  detectUdid, getAndCheckXcodeVersion, getAndCheckIosSdkVersion, getGenericSimulatorForIosVersion,
  checkAppPresent, getDriverInfo,
  clearSystemFiles, waitForClearedSystemFiles, translateDeviceName, normalizeCommandTimeouts,
